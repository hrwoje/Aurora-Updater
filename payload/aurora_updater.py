#!/usr/bin/python3
"""Aurora component updater: hash-checked, staged, and allowlisted."""
from __future__ import annotations
import argparse, hashlib, json, os, re, shutil, subprocess, sys, tempfile, time, urllib.request, threading
from pathlib import Path

REPO = "https://raw.githubusercontent.com/hrwoje/Aurora-Updater/main"
MANIFEST_URL = os.environ.get("AURORA_UPDATER_MANIFEST_URL", f"{REPO}/manifest.json")
STATE = Path(os.environ.get("XDG_STATE_HOME", Path.home() / ".local/state")) / "aurora-updater"
INSTALLED, BACKUPS = STATE / "installed.json", STATE / "backups"
ALLOWED_PREFIXES = tuple(Path.home() / x for x in (".local/bin", ".config/aurora", ".config/autostart", ".local/share/applications", ".local/share/icons", ".local/share/fonts", ".config/gtk-3.0", ".config/gtk-4.0"))
...
def missing_packages(data):
    result = []
    for package in data.get("required_packages", []):
...
def check():
    data, old, changed = manifest(), installed(), []
    for item in data.get("files", []):
        target = target_path(item["target"])
        if not target.exists() or hashlib.sha256(target.read_bytes()).hexdigest() != item["sha256"]: changed.append(item["target"])
    missing = missing_packages(data)
    emit("result", ok=True, version=data["version"], previous=old.get("version"), release_notes=data.get("release_notes", ""), files=changed, packages=missing, update_available=bool(changed or missing or old.get("version") != data["version"]))
    return 0

def install():
    data = manifest(); files, missing = data.get("files", []), missing_packages(data)
    total, done, stamp = len(files) + bool(missing), 0, time.strftime("%Y%m%d-%H%M%S")
    STATE.mkdir(parents=True, exist_ok=True); BACKUPS.mkdir(parents=True, exist_ok=True)
    stage, backup_dir = Path(tempfile.mkdtemp(prefix="aurora-update-", dir=STATE.parent)), BACKUPS / stamp
    try:
        emit("status", message="Manifest opgehaald en gecontroleerd")
        ...
        for item in files:
            ...
            out = stage / source; out.parent.mkdir(parents=True, exist_ok=True); out.write_bytes(raw); done += 1
            emit("progress", fraction=done / max(total, 1), message=f"Gedownload: {source}")
        if missing:
            emit("status", message="Aurora-afhankelijkheden installeren via XBPS")
            result = subprocess.run(["pkexec", "xbps-install", "-y", *missing], text=True, capture_output=True)
            if result.returncode: raise RuntimeError((result.stderr or result.stdout or "XBPS-installatie mislukt").strip())
            done += 1; emit("progress", fraction=done / max(total, 1), message="Afhankelijkheden geïnstalleerd")
        ...
        INSTALLED.write_text(json.dumps({"version": data["version"], "installed_at": stamp, "manifest": MANIFEST_URL}, indent=2), encoding="utf-8")
        emit("progress", fraction=1.0, message="Aurora-update voltooid"); emit("result", ok=True, version=data["version"], backup=str(backup_dir), update_available=False); notify("Aurora is bijgewerkt", f"Aurora componentversie {data['version']} is geïnstalleerd."); return 0
    except Exception as exc:
        emit("result", ok=False, error=str(exc), backup=str(backup_dir) if backup_dir.exists() else None); notify("Aurora-update mislukt", str(exc)); return 1
